...
def define_dataloader(logger, opt):
    ...
    ''' val_dataloader now use DistributedSampler, originally run only GPU 0! '''
...
def define_dataset(logger, opt):
    ...
    dataset_opt = opt['datasets'][opt['phase']]['which_dataset_train']
    phase_dataset = init_obj(dataset_opt, logger, default_file_name='data.dataset', init_type='Dataset')
    
    dataset_opt = opt['datasets'][opt['phase']]['which_dataset_val']
