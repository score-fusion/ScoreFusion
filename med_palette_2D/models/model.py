...
class Palette(BaseModel):
    def __init__(self, networks, losses, sample_num, task, optimizers, ema_scheduler=None, set_device = True, **kwargs):
        ...
        self.set_device_flag = set_device
        ...
        ''' set device is False if we just want inference for 3D model the device is handled in the whole ensemble model'''
        if self.set_device_flag:
        ...
        # if self.set_device_flag:
        #     self.optG.param_groups[0]['capturable'] = True

        self.optimizers.append(self.optG)
        # if self.set_device_flag:
        #     for optimizer in self.optimizers:
        #         print("fix a bug for torch 1.12, remove this if unnecessary")
        ...
        if self.opt['distributed'] and self.set_device_flag:
    ...
    def set_input(self, data):
        ...
        # print("cond_image:",self.cond_image.shape)
    ...
    def get_current_visuals(self, phase='train'):
        dict = {
            ...
            'cond_image': (self.cond_image.detach()[:,:1].float().cpu()+1)/2,
        }
    ...
    def save_current_results(self):
        ...
        for idx in range(self.batch_size):
            ret_path.append('{}_GT'.format(self.path[idx]))
            ret_result.append(self.gt_image[idx].detach().float().cpu())

            ret_path.append('{}_input'.format(self.path[idx]))
            ret_result.append(self.cond_image[idx,:1].detach().float().cpu())

            ret_path.append('{}_Process'.format(self.path[idx]))
            ret_result.append(self.visuals[idx::self.batch_size].detach().float().cpu())
            
            ret_path.append('{}_Out'.format(self.path[idx]))
            ret_result.append(self.visuals[idx-self.batch_size].detach().float().cpu())
    ...
    def train_step(self):
        ...
        train_loss_epoch = []
        if not self.opt['path']['resume_state'] is None:
            self.optG.param_groups[0]['capturable'] = True
        ...
        for train_data in tqdm.tqdm(self.phase_loader):
            ...
            # loss.backward()
            # self.optG.step()
            loss = torch.zeros((), device=self.cond_image.device)
        ...
        for scheduler in self.schedulers:
            scheduler.step()
        print("train loss epoch:",self.train_metrics.result())
        print("train loss epoch avg:", np.mean(train_loss_epoch))
    ...
    def val_step(self):
        ...
        with torch.no_grad():
            # save several viz results
            for num_viz,val_data in enumerate(tqdm.tqdm(self.val_loader)):
                if num_viz > 2:
                    break
            ...
            print("val loss epoch:", np.mean(val_loss_epoch))
            self.writer.add_scalar("val loss epoch", np.mean(val_loss_epoch))
