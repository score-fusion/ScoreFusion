...
class model_ensemble(nn.Module):
    def __init__(self, model_3D, model2D_2, model2D_3, 
        ...
        if baseline == "3D_feature":
            ...
            self.feature_extract_layer_2_10 = zero_module(nn.Conv3d(64, 64, 3, stride = 1, padding = 1))
            ...
            self.feature_extract_layer_3_10 = zero_module(nn.Conv3d(64, 64, 3, stride = 1, padding = 1))
...
class model_ensemble_2cond_3model(nn.Module):
    def __init__(self, model_3D, model2D_1, model2D_2, model2D_3,
                 ...
                 ntime_steps_2D = 1000, 
                 baseline = "3D"):
        super(model_ensemble_2cond_3model, self).__init__()
        self.ntime_steps_2D = ntime_steps_2D
        self.model_3D = model_3D
        ...
        self.model_cond2_1 = model_cond2_1.netG
        ...
        if hasattr(self.model2D_3, "module"):
            ...
            self.model2D_2 = self.model2D_2.module
            self.model_cond2_1 = self.model_cond2_1.module
        ...
        self.model2D_3.denoise_fn.eval()
        self.model_cond2_1.denoise_fn.eval()
        ...
        for param in self.model2D_3.denoise_fn.parameters():
            param.requires_grad = False
        for param in self.model_cond2_1.denoise_fn.parameters():
            param.requires_grad = False
    ...
    def forward(self, x, timesteps):
        ...
        with torch.no_grad():
            if self.baseline == "3D_only":
                ...
                x_2D_1 = x_cond1.permute(0, 2, 1, 3, 4).reshape(-1, c, h, w)
                ...
                x_cond2_1 = x_cond2.permute(0, 2, 1, 3, 4).reshape(-1, c, h, w)
                t_2cond_1 = torch.full((b*d,), int(timesteps.item()/self.time_step*self.ntime_steps_2D), device=x_2D_2.device, dtype=torch.long)
                out_cond2_1 = torch.zeros(b*d, self.out_c, h, w, device=x_2D_1.device, dtype=x_2D_1.dtype)
                for i in range(0, b*d, b_2D):
                    out_cond2_1[i:i+b_2D] = self.model_cond2_1.denoise_inference(x_cond2_1[i:i+b_2D,:self.out_c], t_2cond_1[i:i+b_2D], y_cond=x_cond2_1[i:i+b_2D,self.out_c:])
                out_cond2_1 = out_cond2_1.reshape(b, d, self.out_c, h, w).permute(0, 2, 1, 3, 4)
            else: 
                # 3D features are not implemented
        ...
        if self.baseline == "2D":
            ...
            w5 = 0.1667 #0.25
            w6 = 0.1667
            return w1 * out_2D_1 + w2 * out_2D_2 + w3 * out_2D_3 + w4 * out_cond2_2 + w5 * out_cond2_3 + w6 * out_cond2_1
        elif self.baseline == "TPDM":
            w2 = 0.25
            w3 = 0.5
            w4 = 0.75
            ...
            # inference for 3D model
            x_3D = torch.cat([x, out_2D_1.detach(), out_2D_2.detach(), out_2D_3.detach(), out_cond2_1.detach(), out_cond2_2.detach(), out_cond2_3.detach()], dim = 1)
            ensemble_w = self.model_3D(x_3D, timesteps)
            # ensemble_w = torch.zeros_like(ensemble_w)
            out_3D = (ensemble_w[:,0] + 0.1667) * out_2D_1 + (ensemble_w[:,1] + 0.1667) * out_2D_2 +  (ensemble_w[:,2] + 0.1667) * out_2D_3 + (ensemble_w[:,3] + 0.1667) * out_cond2_1 + (ensemble_w[:,4]+0.1667) * out_cond2_2 +  (0.1667 - ensemble_w[:,0] - ensemble_w[:,1]- ensemble_w[:,2]- ensemble_w[:,4]) * out_cond2_3 + 0.1*ensemble_w[:,5]
...
class model_ensemble_2cond(nn.Module):
    ...
    def forward(self, x, timesteps):
        ...
        if self.baseline == "2D":
            ...
            ensemble_w = self.model_3D(x_3D, timesteps)
            out_3D =  (ensemble_w[:,0] + 0.25) * out_2D_2 + (ensemble_w[:,1] + 0.25) * out_2D_3 + (ensemble_w[:,2] + 0.25) * out_cond2_2 +  (0.25 - ensemble_w[:,0] - ensemble_w[:,1] - ensemble_w[:,2]) * out_cond2_3 + 0.1*ensemble_w[:,3]
