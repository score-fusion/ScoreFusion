    ...
    def __init__(
        ...
        *,
        ...
        if exists(betas):
            ...
            print("please make sure 3D model's beta schedule is consistent with 2D model in config" )
            print("(at GaussianDiffusion_ensemble in diffusion_model/diffusion_ensemble.py)")
        ...
        if cond2:
            denoise_fn.sqrt_alphas_cumprod = self.sqrt_alphas_cumprod
    ...
    def p_sample_loop(self, shape, condition_tensors = None):
        ...
        for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            if self.with_condition:
                ...
                try:
                    if (not self.debug is None) and len(self.debug) > 0 and i % 300 == 0:
