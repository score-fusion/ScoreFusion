...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def sample(self, batch_size = 2, condition_tensors = None,self_consistency_config = {}, debug = ''):
...
class Trainer(object):
    ...
    def viz(self, cur_images, path, video = True, plot = True, file = True):
        ...
        if plot:
            try:
                ...
                print("plot failed for ", path)
        
        if video:
            try:
                make_video(sampleImage, path)
            except:
                print("video failed for ", path)
                ...
                with torch.no_grad():
                    with torch.autocast(device_type="cuda", dtype=torch.float16, enabled = self.fp16):
                        ...
                        if self.with_condition:
                            ...
                            all_images = self.ema_model.module.sample(batch_size=1, condition_tensors=input_data, self_consistency_config = self.self_consistency_config, debug = '')
                        else:
    ...
    def validate(self):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(tqdm(self.val_dl)):
                ...
                if self.with_condition:
                    print("start inference with condition on GPU ", self.gpu_id)
                    all_images = self.ema_model.module.sample(batch_size=batch_size, condition_tensors=input_tensors, debug = '')
                else:
                ...
                for j in range(len(all_images)):
                    ...
                    for k in range(all_images.shape[1]):
                        ...
                        print(np.min(cur_input), np.max(cur_input), np.mean(cur_input))
                        # save res to disk
                        ...
                        cur_path = str(self.results_folder / f'sample-{i*b + j}-GPU{self.gpu_id}-input-modal-{k}')
                        np.save(cur_path, cur_input)
    ...
    def fast_sample(self,sampling_step = 50):    
        ...
        with torch.no_grad():
            ...
            # folders_order = []
            for i, data in enumerate(tqdm(self.val_dl)):
                print(i, data['cur_folder'])
                # folders_order.append(data['cur_folder'])
                with torch.autocast(device_type="cuda", dtype=torch.float16, enabled = self.fp16):
                    if self.fp16:
                        input_tensors = data['input'].half().cuda()
                        target_tensors = data['target'].half().cuda()
                        mask = data['mask'].half().cuda()
                        wrap = Wrap(self.ema_model.module.denoise_fn, input_tensors,fp16 = self.fp16).half().cuda()
                    else:
                        input_tensors = data['input'].cuda()
                        target_tensors = data['target'].cuda()
                        mask = data['mask'].cuda()
                        wrap = Wrap(self.ema_model.module.denoise_fn, input_tensors,fp16 = self.fp16).cuda()
                ...
                for j in range(len(all_images)):
                    ...
                    for k in range(all_images.shape[1]):
                        ...
                        # cur_res = scale(cur_res.cpu().numpy())
                        # cur_target = scale(cur_target.cpu().numpy())
                        cur_input = input_tensors[j].cpu().numpy()
                    ...
                    print(f"GPU{self.gpu_id}")
                    print(total_metrics)
                    print("TOTAL PSNR: ", - 10 * np.mean(np.log10(total_metrics)))
            # print("folders_order", folders_order)
