...
from diffusion_model.unet_brats import create_model, model_ensemble_three_model
...
def main(rank: int, world_size: int, args):
    ...
    try: 
        ...
        if len(resume_weight) > 0 or args.baseline == "3D_feature":
        ...
        #     return item_opt
        networks_2D_1 = [define_network(phase_logger, model_2D_1_opt, item_opt) for item_opt in model_2D_2_opt['model']['which_networks']]
        ...
        model_3D = create_model(input_size_model, 
            ...
            out_channels=out_channels + 2, # for 3 models, 3D model needs to output 3 channels to ensemble 3 2D models, 
        ...
        model = model_ensemble_three_model(model_3D, model2D_1, model2D_2, model2D_3, batch_size_2D_inference = batch_size_2D_inference, time_step=args.timesteps, baseline=args.baseline,weight_3D = args.weight_3D).cuda()
        ...
        if len(resume_weight) > 0:
            ...
            if not args.zero_init_feature:
                diffusion.load_state_dict(weight['ema'])
            else:
                weight = weight['ema']
                our_weights = diffusion.state_dict()
                for k in our_weights.keys():
                    if k not in weight.keys():
                        weight[k] = our_weights[k]
        ...
        if not args.validation:
            if len(resume_weight) > 0:
                ...
                if not args.zero_init_feature:
                    trainer.model.module.load_state_dict(data['model'])
                    trainer.ema_model.module.load_state_dict(data['ema'])
                else:
                    weight_model = data['model']
                    our_weights = trainer.model.module.state_dict()
                    for k in our_weights.keys():
                        if k not in weight_model.keys():
                            weight_model[k] = our_weights[k]
                    
                    weight_ema = data['ema']
                    our_weights = trainer.ema_model.module.state_dict()
                    for k in our_weights.keys():
                        if k not in weight_ema.keys():
                            weight_ema[k] = our_weights[k]
                    
                    trainer.model.module.load_state_dict(weight_model)
                    trainer.ema_model.module.load_state_dict(weight_ema)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--weight_3D', type=float, default=1.0)
    parser.add_argument('--zero_init_feature', action='store_true')
