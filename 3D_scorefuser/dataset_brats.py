...
# from  data.util.utils.dtypes_brats import LabelEnum
...
# from data.util.utils.util import bbox2mask, random_bbox
# from data.util.utils.fft import clear, get_mask, fft2, ifft2
import math
...
def reconstruct_ct_images(sinograms, theta, filter_name='ramp'):
    """
    Reconstruct CT images from sinograms using filtered back projection.
    
    Args:
        sinograms (numpy.ndarray): Array of sinograms
        theta (numpy.ndarray): Array of projection angles
        filter_name (str): Filter to use for reconstruction
        
    Returns:
        numpy.ndarray: Reconstructed images
    """
    print("Reconstructing CT images...")
    reconstructions = []
...
def generate_sparse_view_sinograms(volume, full_angles=180, sparse_angles=36):
    """
    Generate sparse-view sinograms from a CT volume.
    
    Args:
        volume (numpy.ndarray): 3D CT volume
        full_angles (int): Number of angles in full-view reconstruction
        sparse_angles (int): Number of angles for sparse-view reconstruction
        
    Returns:
        tuple: (sparse sinograms, full sinograms, theta_sparse, theta_full)
    """
    print("Generating sinograms...")
    
    # Create angle arrays
    theta_full = np.linspace(0., 180., full_angles, endpoint=False)
    
    # For sparse view, select evenly spaced angles
    step = full_angles // sparse_angles
    theta_sparse = theta_full[::step][:sparse_angles]
    
    # Initialize arrays for sinograms
    ...
    for i in range(num_slices):
        ...
        # Generate full sinogram
        full_sinogram = radon(slice_data, theta=theta_full)
        full_sinograms.append(full_sinogram)
        
        # Generate sparse sinogram by selecting specific projection angles
        sparse_indices = [np.where(theta_full == angle)[0][0] for angle in theta_sparse]
        sparse_sinogram = full_sinogram[:, sparse_indices]
        sparse_sinograms.append(sparse_sinogram)
    
    return np.array(sparse_sinograms), np.array(full_sinograms), theta_sparse, theta_full
...
class NiftiPairImageGenerator(Dataset):
    def __init__(self,
        ...
        # print(os.path.join(dataset_folder, '*'))
        if train:
        ...
        self.chaos_crop_size = chaos_crop_size
    ...
    def read_image(self, file_path):
        # if not file_path in self.mem_data:
        #     img = nib.load(file_path).get_fdata()
        #     self.mem_data[file_path] = img
        # # img = nib.load(file_path).get_fdata()
        # return self.mem_data[file_path]
        img = nib.load(file_path).get_fdata()
        return img
    ...
    def downsample(self, img, downsample,mask = None):
        ...
        if downsample == 'bicubic':
            ...
            return img, mask
        elif downsample == "sparse":
            downsampling_factor = int(downsampling_factor or 60)
            # full angle is 180
            sparse_sinograms, full_sinograms, theta_sparse, theta_full = generate_sparse_view_sinograms(img, 180, downsampling_factor)
            sparse_reconstructions = reconstruct_ct_images(sparse_sinograms, theta_sparse)
            img = sparse_reconstructions
            return img, mask
    ...
    def read_image_chaos(self, folder):
        dicom_files = sorted(glob(os.path.join(folder + "/DICOM_anon/", "*.dcm")))
        slices = []
        for file in dicom_files:
            dicom_data = pydicom.dcmread(file)
            cur_slice = torch.from_numpy(dicom_data.pixel_array.astype(np.float32)).float().unsqueeze(0).unsqueeze(0)
            # print(cur_slice.shape)
            cur_slice = torch.nn.functional.avg_pool2d(cur_slice, 2,2).squeeze().numpy() #resize(,(256, 256), )
    ...
    def get_modality(self, folder, modality, mask = None, idx = None):
        # if idx is not None:
        #     cache_name = "./cache/" + modality + str(idx)
        #     if idx is not None and os.path.isfile(cache_name+".npz"):
        #         try:
        #             img = np.load(cache_name+".npz")['arr_0']
        #             # make_plot(img, "./viz/" + modality + str(idx))
        #             return img, mask
        #         except Exception as e:
        #             print(e)
        #             print(cache_name+".npz")
            # print(cache_name)
        ...
        if modality in ['t1', 't1ce', 't2', 'flair', 'seg']:
            ...
            if len(input_file) != 1:
                ...
                raise ValueError("no file or more than 1 files:" + os.path.join(folder, '*'+modality+'.nii.gz'))
            img = self.read_image(input_file[0])
        elif modality == "HPCt2":
            input_file = glob(os.path.join(folder, '*registered_T2.nii.gz'))
            if len(input_file) != 1:
                print("no file or more than 1 files", input_file)
                print(os.path.join(folder, '*'+modality+'.nii.gz'))
                raise NotImplementedError
            ...
            # input_file = glob(os.path.join(folder, '*'))
            img = self.read_image_chaos(folder)
        ...
        elif modality in ['t1', 't1ce', 't2', 'flair','HPCt2']:
            # print(img.shape)
        ...
        # to be discussed, CT data is already normalized when load using a different norm
        if modality in ['t1', 't1ce', 't2', 'flair','HPCt2']:
        ...
        # if idx is not None:
            # save in cache in compressed format
            # np.save(cache_name, img)
            # np.savez_compressed(cache_name, img)
        # make_plot(img, "./viz/" + modality + str(idx))
    ...
    def __getitem__(self, index):
        ...
        for input_m in self.input_modality:
            cur_im, mask = self.get_modality(cur_folder, input_m, mask = mask, idx = index)
            input_data.append(cur_im)
            
        # print(cur_im.shape)
        d,w,h,c = cur_im.shape
        # add global position embedding
        if self.global_pos_emb:
            i, j, k = np.meshgrid(np.arange(d), np.arange(w), np.arange(h), indexing='ij')
            pos_emb = np.zeros((d, w, h, 3))
            pos_emb[..., 0] = i / d
            pos_emb[..., 1] = j / w
            pos_emb[..., 2] = k / h
            pos_emb = pos_emb * 2. - 1.
        ...
        for target_m in self.target_modality:
            cur_im, target_mask = self.get_modality(cur_folder, target_m, idx = index)
            target_data.append(cur_im)
        target_img = np.concatenate(target_data, axis=-1)
        
        # print("input_img shape before crop: ", input_img.shape)
        if self.chaos_crop_size > 0 and "CHAOS" in self.input_modality[0]:
            # randomly crop the first axis:
            # This is because the CHAOS does not have the same size as the input image
            x_start = np.random.randint(0, input_img.shape[0] - self.chaos_crop_size)
            # cur_im = cur_im[x_start:x_start+self.chaos_crop_size, :, : , :]
            input_img = input_img[x_start:x_start+self.chaos_crop_size, :, :, :]
            target_img = target_img[x_start:x_start+self.chaos_crop_size,:, :, :]
            mask = mask[x_start:x_start+self.chaos_crop_size, :, : , :]
        ...
        return {'input':input_img, 'target':target_img, 'mask':mask, 'cur_folder':cur_folder}

# if this is the main file
    
if __name__ == '__main__':
    dataset = NiftiPairImageGenerator(
        dataset_folder='CHAOS/Train_Sets/CT',
        input_modality='CHAOS_sparsex60',
        target_modality='CHAOS',
        input_size=256,
        depth_size=88,
        transform=None,
        target_transform=None,
